...
Remotes.PlaceDefense().OnClientEvent:Connect(function(action, id)
	if action == "BeginPlacement" then
		-- Barbed wire is placed automatically by the server at the player's
		-- feet, so we never enter placement mode for it. Defensive guard in
		-- case an older server still emits BeginPlacement for it.
		if id == "BarbedWire" then return end
		endPlacement(true)
